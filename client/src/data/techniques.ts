...
export const techniques: Technique[] = [
  {
    ...
    shortDescription: "入力値を同じ振る舞いをする同値パーティションに分割し、各パーティションから代表値をテストする技法",
    effectiveness: "高",
    complexity: "低",
    description: "同値分割法（Equivalence Partitioning）は、テスト対象システムへの入力データを「同値パーティション」と呼ばれるグループに分割し、各グループから代表的な値をテストする手法です。同じグループ内の値は同じ振る舞いをすると仮定されるため、すべての値をテストする必要がなく、テストケース数を大幅に削減できます。",
    principles: [
      "入力データを「有効同値パーティション」と「無効同値パーティション」に分割",
      "有効同値パーティション：システムが正常に処理できるデータ値の範囲",
      "無効同値パーティション：エラーを引き起こすデータ値の範囲",
      "各同値パーティションから少なくとも1つの値をテストケースとして選択"
    ],
    suitableCases: [
      "入力範囲が明確に定義されているとき",
      "入力データが範囲で表現できるとき",
    ...
    examples: [
      {
        ...
        steps: {
          title: "同値パーティションの識別",
          items: [
            { label: "有効同値パーティション", value: "18歳～60歳" },
            { label: "無効同値パーティション1", value: "17歳以下" },
            { label: "無効同値パーティション2", value: "61歳以上" }
          ]
        },
        testCases: {
          title: "テストケース設計",
          ...
          rows: [
            ["1", "有効同値パーティション", "30歳", "登録成功"],
            ["2", "無効同値パーティション1", "15歳", "エラーメッセージ表示"],
            ["3", "無効同値パーティション2", "70歳", "エラーメッセージ表示"]
          ]
        }
      },
      {
        ...
        steps: {
          title: "同値パーティションの識別",
          items: [
            { label: "有効同値パーティション1", value: "1～9個（割引なし）" },
            { label: "有効同値パーティション2", value: "10～49個（5%割引）" },
            { label: "有効同値パーティション3", value: "50～99個（10%割引）" },
            { label: "有効同値パーティション4", value: "100個以上（15%割引）" },
            { label: "無効同値パーティション", value: "0個以下" }
          ]
        },
        testCases: {
          title: "テストケース設計",
          headers: ["No.", "テストケース", "入力値", "期待結果"],
          rows: [
            ["1", "有効同値パーティション1", "5個", "割引なし（0%）"],
            ["2", "有効同値パーティション2", "25個", "5%割引"],
            ["3", "有効同値パーティション3", "75個", "10%割引"],
            ["4", "有効同値パーティション4", "150個", "15%割引"],
            ["5", "無効同値パーティション", "-5個", "エラーメッセージ表示"]
          ]
        }
      }
    ],
    applicationSteps: [
      ...
      {
        title: "同値パーティションの識別",
        description: "入力値を有効同値パーティションと無効同値パーティションに分類します。システムの振る舞いが同じと予想される値のグループを特定します。",
        example: "例：\n・有効同値パーティション：18歳～60歳\n・無効同値パーティション1：17歳以下\n・無効同値パーティション2：61歳以上"
      },
      {
        title: "代表値の選択",
        description: "各同値パーティションから代表的な値を少なくとも1つ選びます。これがテストケースとなります。",
        example: "例：\n・有効同値パーティションから「30歳」を選択\n・無効同値パーティション1から「15歳」を選択\n・無効同値パーティション2から「70歳」を選択"
      },
      {
        title: "テストケースの作成",
    ...
    benefits: [
      ...
      {
        title: "保守性の向上",
        description: "要件変更時に、影響を受ける同値パーティションを特定しやすい"
      },
      {
        title: "初心者にも適用しやすい",
    ...
    drawbacks: [
      {
        title: "同値パーティション内での相違点の見落とし",
        description: "同値パーティション内でも振る舞いが異なる特殊なケースを見逃す可能性"
      },
      {
      ...
      {
        title: "要件の解釈によって同値パーティションが異なる",
        description: "テスターによって同値パーティションの分割方法が異なる可能性がある"
      }
    ],
    effectiveUsage: [
      "境界値分析と組み合わせて使用することで、より効果的なテストが可能",
      "複数の入力パラメータがある場合は、デシジョンテーブルやペアワイズテストなど他の技法と併用する",
      "同値パーティションの特定には、要件仕様書や設計書を十分に理解することが重要",
    ...
    relatedTechniques: [
      {
        ...
        name: "境界値分析",
        description: "同値パーティションの境界に焦点を当てたテスト技法"
      },
      {
        id: "decision-table",
        name: "デシジョンテーブル",
  ...
  },
  {
    ...
    category: "blackbox",
    categoryName: "ブラックボックス",
    shortDescription: "有効・無効同値パーティションの境界値に焦点を当てる技法。バグが発生しやすい境界部分を重点的にテスト",
    effectiveness: "高",
    complexity: "中",
    description: "境界値分析は、同値分割法の拡張として位置づけられ、入力データの境界値に焦点を当てるテスト技法です。同値パーティションの境界部分はバグが発生しやすい領域と言われており、この境界値を検証することで効果的にバグを発見できます。",
    principles: [
      "同値パーティションの境界値および境界値の直前・直後の値をテスト",
    ...
    applicationSteps: [
      {
        title: "同値パーティションを特定する",
        description: "まずは同値分割法を用いて、入力データの同値パーティションを特定します。",
        example: "例：成績評価システムの場合、A(90-100)、B(80-89)、C(70-79)、D(60-69)、F(0-59)という5つの同値パーティション"
      },
      {
        title: "境界値を識別する",
        description: "各同値パーティションの境界値を特定します。通常、パーティションの最小値と最大値が境界となります。",
    ...
    relatedTechniques: [
      {
        id: "equivalence-partitioning",
        name: "同値分割法",
        description: "入力データを同値パーティションに分割してテストする技法"
      },
      {
        id: "decision-table",
        name: "デシジョンテーブル",
  ...
  },
  {
    ...
    relatedTechniques: [
      ...
      },
      {
        id: "boundary-value-analysis",
        name: "境界値分析",
        description: "同値パーティションの境界値をテストする技法"
      }
    ]
  },
  {
  ...
  },
  {
    ...
    category: "blackbox",
    categoryName: "ブラックボックス",
    shortDescription: "テスト対象の特性を階層的なツリー構造で分類し、その組み合わせからテストケースを効率的に生成する技法",
    effectiveness: "高",
    complexity: "中",
    description: "クラシフィケーションツリー法（Classification Tree Method）は、テスト対象システムの入力領域を階層的に分類し、その組み合わせからテストケースを体系的に生成するテスト設計技法です。ツリー構造を用いた視覚的な表現により、テスト条件の整理と網羅的なテストケース設計を支援します。",
    principles: [
      "テスト対象の特性や入力値を階層的なツリー構造で分類",
      "各クラスとそのサブクラスを明確に定義",
      "分類の階層構造をツリー図として表現",
      "クラスの組み合わせからテストケースを生成"
    ],
    suitableCases: [
      "複数のパラメータや条件を持つシステム",
      "多様な入力の組み合わせをテストする必要がある場合",
      "視覚的にテスト条件を把握したい場合",
      "体系的なテストケース設計が必要な場合"
    ],
    history: "クラシフィケーションツリー法は、1990年代初頭にドイツのDaimler-Benzの研究者Matthias Grochtmannによって開発されました。当初は自動車のソフトウェアテスト向けに設計されましたが、その後様々な分野のテストに応用されるようになりました。ドイツのテスト手法としてISTQBなどのテスト標準にも取り入れられています。",
    examples: [
      {
        title: "オンラインショッピングサイトの割引計算",
        description: "オンラインショッピングサイトでの割引計算をテストするためのクラシフィケーションツリーを構築します。",
        steps: {
          title: "クラシフィケーションツリーの構築",
          items: [
            { label: "ルートノード", value: "割引計算" },
            { label: "主要クラス", value: "会員種別、購入金額、商品カテゴリー、クーポン利用" },
            { label: "会員種別のサブクラス", value: "一般会員、プレミアム会員、ゴールド会員" },
            { label: "購入金額のサブクラス", value: "5,000円未満、5,000円以上10,000円未満、10,000円以上" },
            { label: "商品カテゴリーのサブクラス", value: "通常商品、セール商品、限定商品" },
            { label: "クーポン利用のサブクラス", value: "なし、通常クーポン、特別クーポン" }
          ]
        },
        testCases: {
          title: "テストケース設計",
          headers: ["No.", "会員種別", "購入金額", "商品カテゴリー", "クーポン利用", "期待される割引率"],
          rows: [
            ["1", "一般会員", "5,000円未満", "通常商品", "なし", "0%"],
            ["2", "プレミアム会員", "5,000円以上10,000円未満", "セール商品", "通常クーポン", "15%"],
            ["3", "ゴールド会員", "10,000円以上", "限定商品", "特別クーポン", "25%"],
            ["4", "一般会員", "10,000円以上", "セール商品", "通常クーポン", "10%"],
            ["5", "プレミアム会員", "5,000円未満", "限定商品", "なし", "5%"]
          ]
        }
      }
    ],
    applicationSteps: [
      {
        title: "テスト対象の特性を特定する",
        description: "テスト対象システムの入力パラメータ、条件、環境変数などの特性を特定します。",
        example: "例：オンラインショッピングでは、会員種別、購入金額、商品カテゴリー、クーポン利用などの特性"
      },
      {
        title: "クラシフィケーションツリーを作成する",
        description: "特定した特性をルートノードとし、各特性のサブクラスをツリー構造で表現します。",
        example: "例：「会員種別」という特性の下に「一般会員」「プレミアム会員」「ゴールド会員」というサブクラスを配置"
      },
      {
        title: "テストケースを生成する",
        description: "ツリーの各レベルからサブクラスを1つずつ選択し、その組み合わせからテストケースを生成します。",
        example: "例：「一般会員」「5,000円未満」「通常商品」「クーポンなし」の組み合わせで1つのテストケース"
      },
      {
        title: "組み合わせの優先順位を決定する",
        description: "全ての組み合わせをテストすることが難しい場合は、重要度や発生頻度などに基づいて優先順位を付けます。"
      },
      {
        title: "テストを実行して結果を評価する",
        description: "生成したテストケースを実行し、期待される結果と実際の結果を比較評価します。"
      }
    ],
    benefits: [
      {
        title: "視覚的な表現",
        description: "テスト条件を階層的なツリー構造で視覚化でき、理解しやすく維持管理が容易"
      },
      {
        title: "体系的なテストケース設計",
        description: "テスト条件の組み合わせを体系的に設計でき、テストの網羅性が向上"
      },
      {
        title: "効率的なテストケース生成",
        description: "多数のパラメータの組み合わせから効率的にテストケースを生成可能"
      },
      {
        title: "ドキュメントとコミュニケーション",
        description: "視覚的な表現によりステークホルダー間のコミュニケーションが円滑になる"
      },
      {
        title: "ツール化の容易さ",
        description: "構造化されているため、ツールによる自動化や最適化が容易"
      }
    ],
    drawbacks: [
      {
        title: "複雑なツリーの管理",
        description: "パラメータや条件が多い場合、ツリーが複雑化し管理が難しくなる"
      },
      {
        title: "すべての組み合わせのテストが困難",
        description: "特性やサブクラスが多い場合、全組み合わせのテストは現実的でなくなる"
      },
      {
        title: "ツリー構造の設計スキルが必要",
        description: "効果的なツリー構造を設計するには経験とスキルが必要"
      },
      {
        title: "特定の相互作用の見落とし",
        description: "単純な組み合わせだけでは特定の条件間の複雑な相互作用を見逃す可能性がある"
      }
    ],
    effectiveUsage: [
      "最初にツリー構造をしっかり設計し、必要に応じて修正する",
      "ペアワイズテストなどの技法と組み合わせて効率的なテストケース削減を図る",
      "ツリー構造が複雑になる場合は、サブツリーに分割して管理する",
      "専用ツールを活用して効率的にツリーを作成・管理する"
    ],
    relatedTechniques: [
      {
        id: "equivalence-partitioning",
  ...
  },
  {
    ...
    relatedTechniques: [
      ...
      },
      {
        ...
        name: "条件網羅",
        description: "複合条件の真/偽の組み合わせをテストする技法"
      },
      {
        id: "path-coverage",
        name: "パス網羅",
  ...
  },
  {
    ...
    relatedTechniques: [
      ...
      },
      {
        id: "equivalence-partitioning",
        name: "同値分割法",
        description: "入力値を同値パーティションに分割するテスト技法"
      },
      {
        id: "boundary-value-analysis",
        name: "境界値分析",
  ...
  {
    id: "vstep",
    name: "VSTeP（可変強度テストパラメータ）",
    category: "blackbox",
    categoryName: "ブラックボックス",
    shortDescription: "異なるパラメータの組み合わせに対して可変の強度レベルを適用し、効率的で柔軟な組み合わせテストを実現する技法",
    effectiveness: "高",
    complexity: "高",
    description: "VSTeP（Variable Strength Test Parameter）は、組み合わせテストにおいて、パラメータの重要度や相互作用の複雑さに応じて異なる強度レベル（t-way）を適用する技法です。すべてのパラメータに同じ強度を適用する従来の組み合わせテストとは異なり、重要なパラメータには高い強度を、相対的に重要でないパラメータには低い強度を適用することで、テストの効率性と効果性を両立させます。",
    principles: [
      "パラメータの重要度に基づいて可変の強度レベルを設定",
      "重要なパラメータには高強度（3-way、4-way）、重要でないパラメータには低強度（2-way）を適用",
      "パラメータ間の相互作用の複雑さを考慮した強度設定",
      "全体のテストケース数を効率的に制御しつつ、重要な組み合わせを網羅"
    ],
    suitableCases: [
      "多数のパラメータを持つシステムで、パラメータの重要度が異なる場合",
      "リソース制約がある中で効果的なテストを行いたい場合",
      "特定のパラメータ間の相互作用が特に重要な場合",
      "段階的なテスト実施で優先度付けが必要な場合"
    ],
    history: "VSTePは、2010年代初頭に組み合わせテストの効率化を目指して開発された比較的新しい技法です。National Institute of Standards and Technology（NIST）やその他の研究機関で研究が進められ、従来の固定強度組み合わせテストの限界を克服するために考案されました。産業界での実用化も進んでおり、特に複雑なシステムのテストで注目されています。",
    examples: [
      {
        title: "Webアプリケーションの設定テスト",
        description: "多数の設定パラメータを持つWebアプリケーションで、パラメータの重要度に応じて異なる強度レベルを適用します。",
        steps: {
          title: "VSTeP設計プロセス",
          items: [
            { label: "パラメータ分類", value: "重要度に基づいてパラメータをグループ化" },
            { label: "強度レベル設定", value: "各グループに適切な強度レベルを割り当て" },
            { label: "制約定義", value: "不可能な組み合わせや依存関係を定義" },
            { label: "テストケース生成", value: "可変強度に基づいてテストケースを生成" }
          ]
        },
        testCases: {
          title: "パラメータ強度設定例",
          headers: ["パラメータグループ", "パラメータ例", "強度レベル", "理由"],
          rows: [
            ["セキュリティ関連", "認証方式、暗号化設定", "3-way", "セキュリティへの影響が大きく、相互作用が重要"],
            ["パフォーマンス関連", "キャッシュ設定、圧縮設定", "3-way", "性能への影響が大きく、組み合わせによる効果が重要"],
            ["UI関連", "テーマ、言語設定", "2-way", "ユーザビリティに影響するが相互作用は比較的単純"],
            ["ログ関連", "ログレベル、出力形式", "2-way", "デバッグ用途であり、相対的に重要度が低い"],
            ["その他設定", "タイムアウト値、リトライ回数", "2-way", "基本的な動作確認で十分"]
          ]
        }
      },
      {
        title: "組み込みシステムの設定テスト",
        description: "IoTデバイスの設定パラメータで、ハードウェア関連とソフトウェア関連で異なる強度を適用します。",
        steps: {
          title: "強度設定の考え方",
          items: [
            { label: "ハードウェア関連", value: "3-way：デバイスの安定性に直接影響" },
            { label: "通信関連", value: "3-way：ネットワーク接続の信頼性が重要" },
            { label: "アプリケーション関連", value: "2-way：基本的な機能動作の確認" },
            { label: "診断・デバッグ関連", value: "2-way：開発・保守用途" }
          ]
        },
        testCases: {
          title: "具体的なテストケース設計",
          headers: ["テストケースID", "ハードウェア設定", "通信設定", "アプリ設定", "強度適用"],
          rows: [
            ["TC001", "CPU周波数：高, メモリ：8GB, 電源：AC", "WiFi：ON, BT：ON, 5G：ON", "ログレベル：INFO", "3-way（HW+通信）+ 2-way（アプリ）"],
            ["TC002", "CPU周波数：中, メモリ：4GB, 電源：バッテリー", "WiFi：OFF, BT：ON, 5G：OFF", "ログレベル：DEBUG", "3-way（HW+通信）+ 2-way（アプリ）"],
            ["TC003", "CPU周波数：低, メモリ：2GB, 電源：AC", "WiFi：ON, BT：OFF, 5G：ON", "ログレベル：ERROR", "3-way（HW+通信）+ 2-way（アプリ）"]
          ]
        }
      }
    ],
    applicationSteps: [
      {
        title: "パラメータの重要度分析",
        description: "テスト対象システムのすべてのパラメータを特定し、ビジネス上の重要度、技術的なリスク、相互作用の複雑さに基づいて分類します。",
        example: "例：セキュリティ関連パラメータは高重要度、UI関連パラメータは中重要度、ログ関連パラメータは低重要度"
      },
      {
        title: "強度レベルの設定",
        description: "各パラメータグループに対して適切な強度レベル（t-way）を設定します。重要度が高いパラメータには3-way以上、重要度が低いパラメータには2-wayを適用します。",
        example: "例：セキュリティ関連は3-way、UI関連は2-way、ログ関連は2-way"
      },
      {
        title: "制約条件の定義",
        description: "実際には存在しない組み合わせや、技術的に不可能な組み合わせを制約条件として定義します。",
        example: "例：特定のハードウェア設定では特定の通信方式が使用できない"
      },
      {
        title: "テストケース生成",
        description: "設定した可変強度レベルと制約条件に基づいて、実際のテストケースを生成します。専用ツールを使用することが推奨されます。",
        example: "例：ACTS、PICT、CATSなどの組み合わせテスト生成ツール"
      },
      {
        title: "テスト実行と評価",
        description: "生成されたテストケースを実行し、結果を評価します。必要に応じて強度レベルの調整や追加テストケースの検討を行います。"
      },
      {
        title: "強度レベルの最適化",
        description: "テスト結果とリソース制約を考慮して、強度レベルの設定を最適化します。継続的改善のプロセスとして実施します。"
      }
    ],
    benefits: [
      {
        title: "効率的なテストケース数の制御",
        description: "重要でないパラメータの強度を下げることで、全体のテストケース数を大幅に削減できる"
      },
      {
        title: "重要な組み合わせへの集中",
        description: "重要なパラメータには高い強度を適用することで、重要な不具合を見逃すリスクを軽減"
      },
      {
        title: "リソースの最適配分",
        description: "限られたテストリソースを重要度に応じて効果的に配分できる"
      },
      {
        title: "段階的テスト実施の支援",
        description: "強度レベルに基づいて段階的にテストを実施し、早期にリスクを特定可能"
      },
      {
        title: "実用性の向上",
        description: "従来の固定強度テストでは現実的でなかった大規模システムにも適用可能"
      }
    ],
    drawbacks: [
      {
        title: "パラメータ重要度の判断が困難",
        description: "適切な重要度分析には深いドメイン知識と経験が必要"
      },
      {
        title: "強度設定の最適化が複雑",
        description: "効果的な強度レベル設定には試行錯誤と調整が必要"
      },
      {
        title: "ツールの複雑性",
        description: "可変強度テストを支援するツールは従来の組み合わせテストツールより複雑"
      },
      {
        title: "設計プロセスの複雑化",
        description: "従来の組み合わせテストと比較して、設計プロセスが複雑になる"
      },
      {
        title: "組み合わせの見落としリスク",
        description: "低強度に設定したパラメータ間の重要な相互作用を見落とす可能性"
      }
    ],
    effectiveUsage: [
      "パラメータの重要度分析に十分な時間をかけ、ドメインエキスパートの意見を取り入れる",
      "最初は保守的な強度設定から始め、経験を積んでから最適化を図る",
      "専用ツールを活用して効率的にテストケースを生成・管理する",
      "テスト結果をフィードバックして強度設定を継続的に改善する",
      "他のテスト技法と組み合わせて包括的なテスト戦略を構築する"
    ],
    relatedTechniques: [
      {
        id: "pairwise-testing",
        name: "ペアワイズテスト",
        description: "すべての2つの要素の組み合わせをカバーする基本的な組み合わせテスト技法"
      },
      {
        id: "combinatorial-testing",
        name: "組み合わせテスト",
        description: "様々な入力の組み合わせをテストする技法群の総称"
      },
      {
        id: "classification-tree",
        name: "クラシフィケーションツリー",
        description: "テスト対象の特性を階層的に分類してテストケースを生成する技法"
      },
      {
        id: "equivalence-partitioning",
        name: "同値分割法",
        description: "入力値を同値パーティションに分割してテストする基本的なブラックボックステスト技法"
      }
    ]
  }
];
